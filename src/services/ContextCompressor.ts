...
export class ContextCompressor {
  ...
  static async compressMessages(
    ...
    const keyPoints = opts.extractKeyPoints ? await this.extractKeyPoints(messages, opts.compressionModel) : []
  ...
  private static categorizeMessages(
    ...
    const middleMessages = messages.slice(systemMessages.length, messages.length - options.preserveRecentMessages)
  ...
  private static async performCompression(messages: Message[], options: Required<CompressionOptions>): Promise<string> {
...
压缩总结：`
  ...
  private static async extractKeyPoints(messages: Message[], model: string): Promise<string[]> {
    if (messages.length === 0) return []
